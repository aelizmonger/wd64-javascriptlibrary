...
// nodes are items in HTML like elements and test.
...
changeAll.forEach(pTag => {
    pTag.innerText = 'My text has changes using a forEach() method.'
}); 
...
/* 
    - innerText simply reference or allows us to change the text of a specified element. Will return only visible test in a 'node'.

    -testContent does the same thing that innerText does, but will return the FULL text of a 'node.
